<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <meta name="csrf-token" content="{{ csrf_token() }}">

        <title>{{ config('app.name', 'Tugasin') }}</title>

        <!-- Fonts -->
        <link rel="preconnect" href="https://fonts.bunny.net">
        <link href="https://fonts.bunny.net/css?family=figtree:400,500,600&display=swap" rel="stylesheet" />

        <!-- Scripts -->
        @vite(['resources/css/app.css', 'resources/js/app.js'])

        <style>
            .flex-1 {
                margin-left: 1rem;
            }

            .dropdown-up {
                position: absolute;
                bottom: 500%;
                transform: translateY(-0.5rem);
                left: 0;
                z-index: 10;
            }

            .sidebar {
                overflow: visible; /* Pastikan konten sidebar tidak terpotong */
                max-width: 250px; /* Sesuaikan lebar sidebar agar konten tidak tersembunyi */
                min-height: 100vh; /* Pastikan sidebar memiliki tinggi penuh */
                padding: 10px; /* Beri ruang ekstra untuk elemen di dalam sidebar */
            }

            .dropdown-trigger {
                position: relative;
            }
            /* Sidebar Kategori Styling */
            .sidebar-category h3 {
                font-size: 0.8rem;
                font-weight: bold;
                color: rgb(0, 0, 0);
                margin-top: 0.5rem; 
                margin-bottom: 0.7em; 
                padding: 0.5rem 1rem;
                background-color: rgba(0, 0, 0, 0.1);
                border-radius: 0.6rem;
                box-shadow: 0px 4px 6px rgba(0, 0, 0, 0.1);
                display: inline-block;
            }

            .sidebar-category ul {
                list-style: none;
                padding: 0;
                margin: 0;
            }

            .sidebar-category li {
                display: flex;
                align-items: center; 
                justify-content: space-between;
                margin-bottom: 0.5rem;
            }

            .sidebar-category a {
                display: flex;
                align-items: center;
                flex-grow: 1; 
                padding-left: 0.2rem;
                padding-bottom: 0.5rem;
                border-radius: 0.75rem;
                text-decoration: none;
                font-size: 0.875rem;
                color: rgb(0, 0, 0);
                transition: background-color 0.3s ease, color 0.3s ease;
            }

            .sidebar-category a:hover {
                background-color: #e5e7eb;
                color: rgb(0, 0, 0);
            }

            .sidebar-category a svg {
                margin-right: 5px;
                fill: rgb(0, 0, 0);
            }

            .sidebar-category form {
                margin-left: 0rem;
            }

            .sidebar-category button {
                background: none;
                border: none;
                cursor: pointer;
                padding: 0;
                display: flex;
                align-items: left;
                margin-left: 0rem;
            }

            .sidebar-category button svg {
                width: 16px;
                height: 16px;
                fill: rgba(255, 0, 0, 0.95);
            }


        .garis-abru {
            border: none;
            height: 1px; 
            background-color: #d1d5db;
            margin: 1rem 0; 
        }
        </style>

    </head>
    
    <body class="overflow-hidden font-sans antialiased bg-gray-100 dark:bg-gray-900">
        <div class="flex h-screen">
            <!-- Sidebar -->
            <div class="w-64 bg-white dark:bg-gray-800 border-r border-gray-100 dark:border-gray-700 flex flex-col min-h-screen justify-between pb-18" style="background-color: rgba(255, 255, 255, 0.7); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); border-top-right-radius: 1rem; border-bottom-right-radius: 1rem;">
                <!-- Sidebar Navigation Links -->
                <div class="px-4 flex flex-col space-y-2">
                    <div class="px-4 py-6">
                        <!-- Logo -->
                        <a href="{{ route('tasks.dashboard') }}">
                            <img src="{{ asset('images/logo-tugasin.png') }}" alt="Your Logo" class="block h-20 w-20">
                        </a>
                    </div>
                    <hr class="garis-abru">
                    <!-- Dashboard Link -->
                    <a href="{{ route('tasks.dashboard') }}" class="block py-2 text-gray-800 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-700 flex items-center p-3 flex-row gap-2">
                    <svg width="28px" height="28px" viewBox="0 -1 159 159" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <g clip-path="url(#clip0)">
                            <path d="M35.0165 132.618C35.867 132.56 36.7174 132.488 37.5678 132.417C39.3318 132.27 41.1551 132.117 42.9373 132.102C47.0141 132.063 51.1637 132.131 55.1761 132.194L58.2652 132.241C58.5308 132.258 58.7937 132.303 59.0493 132.377C57.7067 136.94 55.5617 141.225 52.715 145.033C51.3132 146.95 49.9627 147.629 48.0459 147.378C46.7548 147.231 45.4483 147.304 44.1811 147.591C43.4348 147.733 42.7551 148.115 42.2456 148.679C41.7362 149.244 41.4249 149.96 41.3592 150.718C41.2442 151.692 41.5139 153.05 43.4541 153.983C44.3078 154.419 45.2267 154.713 46.1747 154.853C46.9061 154.943 47.6376 155.038 48.3691 155.133C51.126 155.492 53.9765 155.862 56.8049 155.967C60.9395 156.12 65.1424 156.163 69.2068 156.204C71.6367 156.228 74.0658 156.253 76.4943 156.3C77.0997 156.334 77.7005 156.426 78.2883 156.573C78.5926 156.638 78.8969 156.703 79.1985 156.757C79.2363 156.763 79.274 156.767 79.3123 156.767H95.3347C95.3939 156.767 95.4524 156.759 95.509 156.743C95.7112 156.688 95.9134 156.621 96.1143 156.554C96.4524 156.421 96.8068 156.333 97.1676 156.294C98.089 156.256 99.0357 156.253 99.9505 156.249C101.597 156.272 103.243 156.204 104.881 156.044C106.934 155.851 108.938 155.296 110.798 154.403C111.37 154.165 111.867 153.774 112.234 153.273C112.6 152.772 112.823 152.18 112.879 151.561C112.857 150.929 112.663 150.314 112.319 149.784C111.973 149.253 111.49 148.828 110.921 148.553C109.782 147.939 108.538 147.545 107.254 147.391C106.384 147.284 105.505 147.26 104.63 147.318C104.412 147.326 104.192 147.333 103.973 147.338C103.356 145.704 102.657 144.122 101.978 142.588C100.621 139.519 99.3341 136.608 98.6495 133.31H101.462C104.453 133.31 107.298 133.304 110.145 133.325C110.875 133.331 111.629 133.396 112.356 133.456C112.826 133.496 113.296 133.536 113.767 133.562C114.409 133.598 115.052 133.649 115.695 133.699C117.282 133.857 118.877 133.912 120.472 133.862C123.991 133.671 127.5 133.254 130.768 132.831C134.119 132.397 137.964 131.772 141.194 129.828C152.767 122.857 158.481 112.904 158.167 100.243C157.976 92.4988 157.949 84.6175 157.923 76.9962L157.91 73.2829C157.898 70.1768 157.932 67.0159 157.964 63.9581C158.03 57.9686 158.095 51.7757 157.819 45.6879C157.44 38.587 156.602 31.5184 155.307 24.5268C153.932 16.6996 150.739 10.9909 145.547 7.07367C139.03 2.03978 130.882 -0.39464 122.678 0.24077C115.323 0.748905 108.589 1.08754 102.09 1.27712C95.4192 1.47255 88.6304 1.44662 82.0634 1.42121C72.4488 1.38603 62.5071 1.34814 52.7228 1.99568C48.6623 2.26473 44.5146 2.35655 40.5035 2.44515C36.8676 2.52658 33.1081 2.60934 29.4117 2.82431C22.3069 3.23668 16.1917 5.50909 11.2351 9.5774C8.67482 11.5921 6.53818 14.095 4.94857 16.9415C3.35894 19.7881 2.34779 22.9219 1.97359 26.1621C1.51511 30.693 1.30478 35.2457 1.34354 39.7996L1.33965 40.1801C1.32145 42.0419 1.44761 43.9218 1.56919 45.7406C1.66672 47.1882 1.76426 48.6853 1.79222 50.1504C2.01459 63.1468 2.2129 76.8959 2.41576 93.4193C2.40945 94.1698 2.35755 94.9203 2.26036 95.6649C2.15885 96.4531 2.10396 97.2459 2.0959 98.0407C2.1076 99.2648 2.17586 100.501 2.24218 101.697C2.3202 103.109 2.40144 104.569 2.38584 105.996C2.32927 111.211 4.05881 116.234 7.52702 120.926C13.86 129.48 23.1099 133.422 35.0165 132.618ZM149.471 75.0484L149.475 81.8645C149.481 88.6552 149.488 95.6753 149.439 102.581C149.412 104.063 149.178 105.534 148.746 106.952C147.704 110.495 145.975 113.797 143.659 116.67C140.138 121.025 135.313 123.626 128.907 124.622C125.342 125.176 121.58 125.591 117.403 125.89C116.284 125.933 115.163 125.877 114.054 125.724C113.33 125.647 112.581 125.567 111.836 125.538C109.005 125.428 106.172 125.346 103.34 125.264C97.9355 125.107 92.3464 124.945 86.8684 124.587C78.9099 124.066 70.8655 123.935 62.9115 123.935C58.9316 123.935 54.9732 123.969 51.0602 124C45.185 124.049 39.1101 124.097 33.1393 124.036C28.7439 123.991 25.2354 123.294 22.0969 121.84C19.7861 120.809 17.7088 119.318 15.9914 117.457C14.2741 115.596 12.9525 113.405 12.1071 111.016C10.3853 106.355 9.80274 102.091 10.3262 97.9795C10.4516 96.773 10.4937 95.56 10.4523 94.3476C10.4484 93.9763 10.4438 93.6056 10.4445 93.2363C10.4506 90.2148 10.4592 87.1934 10.4706 84.1713C10.4979 76.1115 10.5258 67.7775 10.4666 59.5789C10.4432 56.3712 10.3314 53.1159 10.2235 49.9668C10.0934 46.2535 9.96334 42.4159 9.97699 38.6472C9.95254 33.0938 11.1799 27.6065 13.5674 22.5947C16.0915 17.3622 20.6547 14.092 27.1398 12.8764C30.952 12.1598 34.7414 11.7754 37.9457 11.492C51.9121 10.2601 66.1645 10.1415 79.9437 10.0268C83.0315 10.0012 86.1193 9.97133 89.2065 9.93746C93.5889 9.88404 97.9713 9.83609 102.353 9.7931C109.303 9.72078 116.489 9.64639 123.558 9.5402C128.495 9.4516 133.336 11.0124 138.807 14.4051C141.945 16.3504 144.139 19.564 145.328 23.9555C146.279 27.4733 146.968 31.1338 147.633 34.6771L147.986 36.55C148.429 38.8757 148.808 41.436 149.144 44.3792C149.349 46.2286 149.45 48.0877 149.447 49.9484C149.471 56.062 149.476 62.1767 149.464 68.2921V74.9317C149.463 74.9702 149.465 75.0086 149.471 75.0471V75.0484ZM63.4044 145.795C64.2896 144.022 65.0613 142.194 65.7146 140.323C66.277 138.454 66.714 136.551 67.0241 134.625C67.1483 133.947 67.2744 133.263 67.4142 132.574L89.9334 132.965C90.308 138.248 91.9439 142.917 93.7489 147.193H62.7484C62.963 146.728 63.184 146.261 63.4044 145.795Z" fill="#000000"/>
                            <path d="M126.331 34.0147C124.846 33.0375 123.481 33.9822 122.825 34.4343C121.586 35.2474 120.442 36.1968 119.413 37.2644C116.267 40.704 113.118 44.2445 110.073 47.6679L107.685 50.35C105.706 52.5715 103.729 54.7944 101.752 57.0188C97.7839 61.48 93.8105 65.9367 89.8313 70.3894C89.116 71.19 88.326 71.9372 87.4905 72.7281C87.222 72.9821 86.9502 73.2408 86.6758 73.5046C86.0978 73.0727 85.5347 72.6466 84.9814 72.2271C83.4651 71.0792 82.0327 69.9959 80.5385 68.9698L79.9293 68.5523C75.8265 65.736 71.5826 62.8232 67.3029 60.1152C63.337 57.6077 60.9371 57.9654 57.7842 61.5399C56.4559 63.0467 55.0092 64.6205 53.0937 66.6455C49.3857 70.5646 45.6759 74.4818 41.964 78.397C36.9358 83.7037 31.9098 89.0131 26.8858 94.325C26.0421 95.2338 25.252 96.1908 24.5197 97.1914C24.2822 97.4644 24.1027 97.7829 23.9921 98.1275C23.8815 98.4722 23.8422 98.8357 23.8765 99.1959C23.9109 99.5562 24.0181 99.906 24.1918 100.224C24.3654 100.541 24.6017 100.82 24.8864 101.043C25.3786 101.537 26.0413 101.822 26.7376 101.84C27.496 101.809 28.2161 101.497 28.7584 100.965C29.9223 99.9295 31.0243 98.8037 32.1102 97.695C35.1237 94.6149 38.1363 91.5335 41.148 88.4515C47.0935 82.3676 53.0446 76.2902 59.0014 70.218C59.9851 69.2167 61.0665 68.2637 62.1127 67.3445C62.4824 67.0194 62.8508 66.6937 63.218 66.3673C63.348 66.4455 63.4736 66.5204 63.5945 66.5927C64.0821 66.8845 64.5048 67.136 64.8897 67.4187L68.6105 70.1333C73.266 73.5287 78.0801 77.0387 82.7765 80.5422C86.3293 83.193 88.7051 83.148 91.8541 80.3605C93.529 78.8771 95.1122 77.2941 96.596 75.6199C104.364 66.8702 110.875 59.5021 117.403 51.9844C119.905 49.1017 122.419 45.9541 124.88 42.6265C125.907 41.1317 126.723 39.5028 127.307 37.7854C127.903 36.1887 127.548 34.8134 126.331 34.0147Z" fill="#000000"/>
                        </g>
                        <defs>
                            <clipPath id="clip0">
                                <rect width="158" height="157" fill="white" transform="translate(0.777344)"/>
                            </clipPath>
                        </defs>
                    </svg>
                        <span class="ml-2 text-black text-sm p-1">{{ __('Dashboard') }}</span>
                    </a>

                    <!-- Daftar Tugas Link -->
                    <a href="{{ route('tasks.index') }}" class="block py-2 text-gray-800 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-700 flex items-center p-3 flex-row gap-2">
                    <svg width="28px" height="28px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M12.37 8.87988H17.62" stroke="#000000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M6.38 8.87988L7.13 9.62988L9.38 7.37988" stroke="#000000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M12.37 15.8799H17.62" stroke="#000000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M6.38 15.8799L7.13 16.6299L9.38 14.3799" stroke="#000000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M9 22H15C20 22 22 20 22 15V9C22 4 20 2 15 2H9C4 2 2 4 2 9V15C2 20 4 22 9 22Z" stroke="#000000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                        <span class="ml-2 text-black text-sm p-1">{{ __('Daftar Tugas') }}</span>
                    </a>

                    <!-- Kalender Link -->
                    <a href="{{ route('tasks.calendar') }}" class="block py-2 text-gray-800 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-700 flex items-center p-3 flex-row gap-2">
                    <svg fill="#000000" width="28px" height="28px" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" d="M17,2 C17.5522847,2 18,2.44771525 18,3 L18,4 L19,4 C20.6568542,4 22,5.34314575 22,7 L22,19 C22,20.6568542 20.6568542,22 19,22 L5,22 C3.34314575,22 2,20.6568542 2,19 L2,7 C2,5.34314575 3.34314575,4 5,4 L6,4 L6,3 C6,2.44771525 6.44771525,2 7,2 C7.55228475,2 8,2.44771525 8,3 L8,4 L16,4 L16,3 C16,2.44771525 16.4477153,2 17,2 Z M4,12 L4,19 C4,19.5522847 4.44771525,20 5,20 L19,20 C19.5522847,20 20,19.5522847 20,19 L20,12 L4,12 Z M4,10 L20,10 L20,7 C20,6.44771525 19.5522847,6 19,6 L18,6 L18,7 C18,7.55228475 17.5522847,8 17,8 C16.4477153,8 16,7.55228475 16,7 L16,6 L8,6 L8,7 C8,7.55228475 7.55228475,8 7,8 C6.44771525,8 6,7.55228475 6,7 L6,6 L5,6 C4.44771525,6 4,6.44771525 4,7 L4,10 Z"/>
                    </svg>
                        <span class="ml-2 text-black text-sm p-1">{{ __('Kalender') }}</span>
                    </a>
                    <hr class="garis-abru">
                    <!-- Sidebar Kategori -->
                    <div class="sidebar-category">
                        <ul>
                            @foreach (\App\Models\Category::all() as $category)
                            <li>
                            <a href="{{ route('tasks.index', ['category' => $category->name]) }}">
                            <svg width="18px" height="18px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M3 8.2C3 7.07989 3 6.51984 3.21799 6.09202C3.40973 5.71569 3.71569 5.40973 4.09202 5.21799C4.51984 5 5.0799 5 6.2 5H9.67452C10.1637 5 10.4083 5 10.6385 5.05526C10.8425 5.10425 11.0376 5.18506 11.2166 5.29472C11.4184 5.4184 11.5914 5.59135 11.9373 5.93726L12.0627 6.06274C12.4086 6.40865 12.5816 6.5816 12.7834 6.70528C12.9624 6.81494 13.1575 6.89575 13.3615 6.94474C13.5917 7 13.8363 7 14.3255 7H17.8C18.9201 7 19.4802 7 19.908 7.21799C20.2843 7.40973 20.5903 7.71569 20.782 8.09202C21 8.51984 21 9.0799 21 10.2V15.8C21 16.9201 21 17.4802 20.782 17.908C20.5903 18.2843 20.2843 18.5903 19.908 18.782C19.4802 19 18.9201 19 17.8 19H6.2C5.07989 19 4.51984 19 4.09202 18.782C3.71569 18.5903 3.40973 18.2843 3.21799 17.908C3 17.4802 3 16.9201 3 15.8V8.2Z" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                                <span>{{ $category->name }}</span>
                            </a>
                            <form action="{{ route('categories.destroy', $category->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus kategori ini?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit">
                                    <svg height="16px" width="16px" version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                                        <g>
                                            <path class="st0" d="M88.594,464.731C90.958,491.486,113.368,512,140.234,512h231.523c26.858,0,49.276-20.514,51.641-47.269
                                            l25.642-335.928H62.952L88.594,464.731z M420.847,154.93l-23.474,307.496c-1.182,13.37-12.195,23.448-25.616,23.448H140.234
                                            c-13.42,0-24.434-10.078-25.591-23.132L91.145,154.93H420.847z"/>
                                            <path class="st0" d="M182.954,435.339c5.877-0.349,10.35-5.4,9.992-11.269l-10.137-202.234c-0.358-5.876-5.401-10.349-11.278-9.992
                                            c-5.877,0.357-10.35,5.409-9.993,11.277l10.137,202.234C172.033,431.231,177.085,435.696,182.954,435.339z"/>
                                            <path class="st0" d="M256,435.364c5.885,0,10.656-4.763,10.656-10.648V222.474c0-5.885-4.771-10.648-10.656-10.648
                                            c-5.885,0-10.657,4.763-10.657,10.648v202.242C245.344,430.601,250.115,435.364,256,435.364z"/>
                                            <path class="st0" d="M329.046,435.339c5.878,0.357,10.921-4.108,11.278-9.984l10.129-202.234c0.348-5.868-4.116-10.92-9.993-11.277
                                            c-5.877-0.357-10.92,4.116-11.277,9.992L319.054,424.07C318.697,429.938,323.17,434.99,329.046,435.339z"/>
                                            <path class="st0" d="M439.115,64.517c0,0-34.078-5.664-43.34-8.479c-8.301-2.526-80.795-13.566-80.795-13.566l-2.722-19.297
                                            C310.388,9.857,299.484,0,286.642,0h-30.651H225.34c-12.825,0-23.728,9.857-25.616,23.175l-2.721,19.297
                                            c0,0-72.469,11.039-80.778,13.566c-9.261,2.815-43.357,8.479-43.357,8.479C62.544,67.365,55.332,77.172,55.332,88.38v21.926h200.66
                                            h200.676V88.38C456.668,77.172,449.456,67.365,439.115,64.517z M276.318,38.824h-40.636c-3.606,0-6.532-2.925-6.532-6.532
                                            s2.926-6.532,6.532-6.532h40.636c3.606,0,6.532,2.925,6.532,6.532S279.924,38.824,276.318,38.824z"/>
                                        </g>
                                    </svg>
                                </button>
                            </form>
                            </li>
                            @endforeach
                        </ul>
                    </div>
                </div>
                <hr class="garis-abru">
                <!-- Bottom Section (Dropdown) -->
                <div class="mt-auto px-4 mb-72">
                    <x-dropdown align="left" width="48">
                        <x-slot name="trigger">
                            <button class="dropdown-trigger inline-flex items-center px-3 py-2 border text-sm leading-4 font-medium rounded-md">
                                <div>{{ Auth::user()->name }}</div>
                                <div class="ms-1">
                                    <svg fill="#000000" height="14px" width="10px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 511.735 511.735">
                                        <path d="M508.788,371.087L263.455,125.753c-4.16-4.16-10.88-4.16-15.04,0L2.975,371.087c-4.053,4.267-3.947,10.987,0.213,15.04 c4.16,3.947,10.667,3.947,14.827,0l237.867-237.76l237.76,237.76c4.267,4.053,10.987,3.947,15.04-0.213 C512.734,381.753,512.734,375.247,508.788,371.087z"/>
                                    </svg>
                                </div>
                            </button>
                        </x-slot>
                        <x-slot name="content">
                            <div class="dropdown-up bg-white dark:bg-gray-800 shadow-md rounded-md">
                                <x-dropdown-link :href="route('profile.edit')">
                                    {{ __('Profile') }}
                                </x-dropdown-link>
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <x-dropdown-link :href="route('logout')" onclick="event.preventDefault(); this.closest('form').submit();">
                                        {{ __('Log Out') }}
                                    </x-dropdown-link>
                                </form>
                            </div>
                        </x-slot>
                    </x-dropdown>
                </div>
            </div>
            
            <!-- Main Content -->
            <div class="flex-1 overflow-auto">
               
                <main>
                    @yield('content')
                </main>
            </div>
        </div>

        
    </body>
</html>
